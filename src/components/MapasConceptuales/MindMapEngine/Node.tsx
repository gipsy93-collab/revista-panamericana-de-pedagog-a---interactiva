...
const NodeComponent: React.FC<NodeProps> = ({ node, isVisible, delay, onClick }) => {
  ...
  return (
    <button
      ...
      className={`absolute flex items-center cursor-pointer select-none transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] font-bold uppercase tracking-wider shadow-black`}
      style={{
        left: `${node.x}px`,
        top: `${node.y}px`,
        transform: `translate(-50%, -50%) scale(${scale})`,
        opacity,
        zIndex: hovered ? 100 : (40 - node.level * 10),
    ...
    >
      <div 
        className="flex-shrink-0 flex items-center justify-center rounded bg-black/5 border border-black/10"
        style={{
      ...
      >
        <IconComponent size={styles.iconSize} color="#000000" />
      </div>
      <span style={{ fontSize: styles.fontSize }}>{node.label}</span>
    </button>
  );
};
