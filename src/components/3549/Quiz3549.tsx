      ...
      <section className="py-20 px-6 bg-black text-white min-h-[60vh] flex items-center justify-center">
        <BrutalCard color="bg-white" className="max-w-2xl w-full text-center py-16" shadowColor={ARTICLE_META.themeColor}>
          <div className="flex flex-col gap-8 items-center">
            <BrutalSticker text="ARQUEOLOGÍA COMPLETADA" color="bg-[#4f46e5]" className="text-white" />
            <h3 className="text-6xl font-display uppercase text-black">
              {score} / {QUIZ_QUESTIONS.length}
            </h3>
            <p className="text-xl font-sans text-black/70">
              Has validado los fundamentos teóricos de la Nueva Escuela Mexicana.
            </p>
            <button 
              onClick={restartQuiz}
            ...
            >
              <RotateCcw size={20} /> Reiniciar Examen
            </button>
          </div>
        </BrutalCard>
      </section>
    ...
    <section className="py-20 px-6 bg-[#f0f0f0]">
      <div className="max-w-4xl mx-auto flex flex-col gap-12">
        <div className="flex flex-col gap-4 text-center items-center">
          <BrutalSticker text="EXAMEN DE TERRENO" color="bg-black" className="text-white" />
          <PremiumTitle subtitle={`PEDAGOGÍA CRÍTICA // ${currentQuestion + 1} / ${QUIZ_QUESTIONS.length}`}>
            Validación de Conceptos
          </PremiumTitle>
        </div>

        <BrutalCard className="flex flex-col gap-8 p-12">
          <div className="flex items-center gap-4 text-[#4f46e5] mb-2 font-mono text-sm font-black uppercase">
            <Brain size={24} /> Desafío Teórico
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {question.options.map((opt, idx) => {
              ...
              if (showFeedback) {
                if (isCorrect) variantStyle = "bg-indigo-100 border-indigo-600 text-indigo-900";
              ...
              return (
                ...
                >
                  {opt}
                  {showFeedback && isCorrect && <CheckCircle2 className="text-indigo-600" />}
          ...
          <AnimatePresence>
            {showFeedback && (
              <motion.div 
                ...
                className={`p-6 border-4 border-black border-l-[16px] ${selectedOption === question.correctAnswer ? 'bg-indigo-50 border-indigo-600' : 'bg-red-50 border-red-600'}`}
              >
                ...
                <button 
                  ...
                  className="mt-6 flex items-center gap-2 bg-black text-white px-6 py-3 font-display uppercase self-end ml-auto shadow-[4px_4px_0_0_#4f46e5]"
                >
