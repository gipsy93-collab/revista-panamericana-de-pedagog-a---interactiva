...
export default function Timeline() {
  return (
    <section className="relative bg-[#f5ece0] pt-48 pb-64 px-6 md:px-12 flex flex-col items-center justify-center overflow-hidden z-10">
      <div className="absolute z-0 text-center font-pop text-[90px] md:text-[200px] leading-none text-[#fccb06] opacity-40 pointer-events-none w-full flex flex-col pt-20" style={{ WebkitTextStroke: 'calc(var(--brutal-stroke-width) * 2) white' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-16 items-start z-10 w-full max-w-[1400px]">
        {pedagogues.map((p, idx) => (
          ...
          >
            ...
            <h3 className="font-pop text-5xl text-white mb-2" style={{WebkitTextStroke: 'var(--brutal-stroke-width) black'}}>{p.name}</h3>
