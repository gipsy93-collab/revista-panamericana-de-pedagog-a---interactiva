...
export const Quiz3290 = ({ onComplete }: { onComplete: (result: QuizResult) => void }) => {
  ...
  if (isFinished) {
    ...
    return (
      <div className="py-24 px-6 bg-indigo-50/50 min-h-[60vh] flex items-center justify-center">
        <BrutalCard color={passed ? "bg-black" : "bg-rose-600"} className="max-w-2xl w-full text-center p-12">
          <Trophy size={80} className={`mx-auto mb-6 ${passed ? 'text-indigo-400' : 'text-white'}`} />
          <h2 className="text-5xl font-display uppercase mb-4 text-white">
            {passed ? '¡Desafío Superado!' : 'Odisea Fragmentada'}
          </h2>
          <p className="text-2xl font-serif text-white/90 mb-8">
            Puntuación Académica: {score} / {QUIZ_QUESTIONS.length}
          </p>
          
          <div className="inline-block p-4 border-4 border-white bg-indigo-600 text-white shadow-[6px_6px_0_#fff] font-mono text-xl uppercase font-black mb-8 rotate-[-2deg]">
            +{score * 100} XP MULTIMODAL
          </div>

          <p className="font-sans text-lg text-white mb-8">
            {passed 
              ? 'Has demostrado una comprensión profunda sobre las tensiones de la multimodalidad en Colombia. Conoces la diferencia entre tener la herramienta y saber integrarla.' 
              : 'Los resultados sugieren una desconexión con los hallazgos fenomenológicos. Repasa la importancia de la interacción presencial-virtual para optimizar tu reporte.'}
          </p>
          ...
          {!passed && (
            ...
            >
              <RefreshCcw /> Reintentar Análisis
            </button>
          )}
        </BrutalCard>
      </div>
    ...
    <div className="py-24 px-6 md:px-12 bg-zinc-50 min-h-[80vh]">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-12">
          <BrutalSticker text="DIAGNÓSTICO ACADÉMICO" color="bg-indigo-400" className="text-white" />
          <div className="font-mono text-xl font-black bg-black text-white px-4 py-2 shadow-[4px_4px_0_#4f46e5]">
        ...
        <AnimatePresence>
          {showExplanation && (
            ...
            >
              <div className="p-8 border-4 border-black bg-indigo-50 shadow-[8px_8px_0_#000]">
                <h4 className="font-display text-2xl uppercase mb-2 flex items-center gap-3">
                  <Target size={24} /> Justificación Teórica
                </h4>
                ...
                >
                  {currentQuestion < QUIZ_QUESTIONS.length - 1 ? 'Siguiente Desafío →' : 'Finalizar Odisea'}
                </button>
              </div>
            </motion.div>
          )}
