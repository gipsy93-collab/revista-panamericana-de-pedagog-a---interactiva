    ...
    <div className="relative min-h-screen bg-white text-zinc-900 font-sans pt-20 overflow-x-hidden selection:bg-indigo-600 selection:text-white">
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] bg-white border-b-2 border-black/10 backdrop-blur-md bg-white/90">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <button
            onClick={onBack}
            className="group flex items-center gap-3 bg-black text-white px-6 py-3 border-4 border-black hover:bg-indigo-600 transition-colors shadow-[4px_4px_0_0_#3730a3] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0_0_#3730a3] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
          >
          ...
          <div className="hidden md:flex items-center gap-4">
             <div className="bg-indigo-600 text-white px-4 py-2 border-4 border-black font-display text-xl font-black italic shadow-[4px_4px_0_0_#000]">N° 39</div>
             <span className="font-mono text-[10px] uppercase font-black tracking-widest opacity-40">REF 3290 // HYBRID ODYSSEY</span>
          </div>
        </div>
      </header>
      ...
      <footer className="bg-black py-32 px-6 border-t-[20px] border-zinc-900 text-center relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-white">
          ...
          <p className="font-serif text-xl italic text-slate-400 mb-12 leading-relaxed text-center">
            Roldán-Morales, C. A. & Torres-Tovar, A. F. (2025). <span className="text-white not-italic font-bold">Desafíos en el aprendizaje híbrido en una universidad del suroccidente colombiano</span>. <span className="text-indigo-500">Revista Panamericana de Pedagogía</span>, 39, e3290.
          </p>
          <div className="flex flex-col md:flex-row justify-center items-center gap-10">
             <div className="bg-white text-black px-8 py-4 border-4 border-black font-mono text-sm font-black uppercase tracking-widest shadow-[8px_8px_0_0_#3730a3]">
               DOI: 10.21555/rpp.3290
             </div>
             <p className="font-mono text-[10px] uppercase opacity-40">Colombia // RPP N° 39</p>
          </div>
        </div>
      </footer>
    </div>
