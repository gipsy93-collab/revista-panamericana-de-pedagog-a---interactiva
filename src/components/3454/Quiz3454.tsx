...
export const Quiz3454 = ({ onComplete }: { onComplete: (result: QuizResult) => void }) => {
  ...
  if (isFinished) {
    ...
    return (
      <div className="py-24 px-6 bg-[#ef4444]/10 min-h-[60vh] flex items-center justify-center border-t-8 border-black">
        <BrutalCard color={passed ? "bg-black" : "bg-[#1e1b4b]"} className="max-w-2xl w-full text-center p-12 shadow-[12px_12px_0_#ef4444]">
          ...
          <h2 className="text-5xl font-display uppercase mb-4 text-white">
            {passed ? '¡Mediador de Paz!' : 'Conflicto de Datos'}
          </h2>
          <p className="text-2xl font-serif text-white/90 mb-8">
            Puntuación Diagnóstica: {score} / {QUIZ_QUESTIONS.length}
          </p>
          
          <div className="inline-block p-4 border-4 border-white bg-[#ef4444] text-white shadow-[6px_6px_0_#fff] font-mono text-xl uppercase font-black mb-8 rotate-[-2deg]">
            +{score * 100} XP PAZ URBANA
          </div>

          <p className="font-sans text-lg text-white mb-8">
            {passed 
              ? 'Excelentes métricas. Has descifrado la brecha sistémica que sufren los alumnos mexicanos entre un entorno bélico externo y las políticas abstractas universitarias.' 
              : 'Resultados difusos. Es crucial que repases la percepción métrica que tiene el bloque de estudiantes sobre el impacto real que tiene la criminalidad en sus horas de aprendizaje.'}
          </p>
          ...
          {!passed && (
            ...
            >
              <RefreshCcw /> Reintentar Evaluación
            </button>
          )}
        </BrutalCard>
      </div>
    ...
    <div className="py-24 px-6 md:px-12 bg-zinc-50 min-h-[80vh] border-t-8 border-black">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-12">
          <BrutalSticker text="SIMULADOR DE CRISIS" color="bg-[#facc15]" />
          <div className="font-mono text-xl font-black bg-black text-white px-4 py-2 shadow-[4px_4px_0_#ef4444]">
            ...
            >
              <div className="p-8 border-4 border-black bg-[#facc15] shadow-[8px_8px_0_#000]">
                <h4 className="font-display text-2xl uppercase mb-2 flex items-center gap-3">
                  <Target size={24} /> Observación Analítica
                </h4>
